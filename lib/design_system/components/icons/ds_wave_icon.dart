...
class DSWaveIcon extends StatefulWidget {
  ...
  const DSWaveIcon({super.key, this.isAnimated = true});
    ...
    return DecoratedBox(
      ...
      child: SizedBox(
        ...
        child: _controller != null
            ...
            : CustomPaint(painter: _PlayWavesPainter(progress: null)),
      ),
    );
  }
}
...
class _PlayWavesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 🔹 Core Center Speaker Icon (Replaced Triangle)
    // Scaled & centered to fit within our 44x44 space beautifully
    final speakerPath = Path();
    const double scale = 1.1; // Balanced scale factor for visual harmony

    speakerPath.moveTo(
      center.dx + (10.5439 - 12.0) * scale,
      center.dy + (8.93982 - 11.8285) * scale,
    );
    speakerPath.cubicTo(
      center.dx + (10.9445 - 12.0) * scale,
      center.dy + (8.66958 - 11.8285) * scale,
      center.dx + (11.3924 - 12.0) * scale,
      center.dy + (8.91381 - 11.8285) * scale,
      center.dx + (12.2856 - 12.0) * scale,
      center.dy + (9.40271 - 11.8285) * scale,
    );
    speakerPath.cubicTo(
      center.dx + (12.5175 - 12.0) * scale,
      center.dy + (9.52955 - 11.8285) * scale,
      center.dx + (12.7413 - 12.0) * scale,
      center.dy + (9.66012 - 11.8285) * scale,
      center.dx + (12.939 - 12.0) * scale,
      center.dy + (9.78504 - 11.8285) * scale,
    );
    speakerPath.cubicTo(
      center.dx + (13.1642 - 12.0) * scale,
      center.dy + (9.92741 - 11.8285) * scale,
      center.dx + (13.4098 - 12.0) * scale,
      center.dy + (10.0987 - 11.8285) * scale,
      center.dx + (13.6567 - 12.0) * scale,
      center.dy + (10.2802 - 11.8285) * scale,
    );
    speakerPath.cubicTo(
      center.dx + (14.5521 - 12.0) * scale,
      center.dy + (10.2661 - 11.8285) * scale,
      center.dx + (15.0 - 12.0) * scale,
      center.dy + (11.2661 - 11.8285) * scale,
      center.dx + (15.0 - 12.0) * scale,
      center.dy + (11.8285 - 11.8285) * scale,
    );
    speakerPath.cubicTo(
      center.dx + (14.9999 - 12.0) * scale,
      center.dy + (12.3907 - 11.8285) * scale,
      center.dx + (14.5522 - 12.0) * scale,
      center.dy + (12.7194 - 11.8285) * scale,
      center.dx + (13.6567 - 12.0) * scale,
      center.dy + (13.3768 - 11.8285) * scale,
    );
    speakerPath.cubicTo(
      center.dx + (13.4097 - 12.0) * scale,
      center.dy + (13.5582 - 11.8285) * scale,
      center.dx + (13.1642 - 12.0) * scale,
      center.dy + (13.7295 - 11.8285) * scale,
      center.dx + (12.939 - 12.0) * scale,
      center.dy + (13.872 - 11.8285) * scale,
    );
    speakerPath.cubicTo(
      center.dx + (12.7415 - 12.0) * scale,
      center.dy + (13.9967 - 11.8285) * scale,
      center.dx + (12.5173 - 12.0) * scale,
      center.dy + (14.126 - 11.8285) * scale,
      center.dx + (12.2856 - 12.0) * scale,
      center.dy + (14.2528 - 11.8285) * scale,
    );
    speakerPath.cubicTo(
      center.dx + (11.392 - 12.0) * scale,
      center.dy + (14.7419 - 11.8285) * scale,
      center.dx + (10.9446 - 12.0) * scale,
      center.dy + (14.9864 - 11.8285) * scale,
      center.dx + (10.5439 - 12.0) * scale,
      center.dy + (14.7157 - 11.8285) * scale,
    );
    speakerPath.cubicTo(
      center.dx + (10.1433 - 12.0) * scale,
      center.dy + (14.4449 - 11.8285) * scale,
      center.dx + (10.107 - 12.0) * scale,
      center.dy + (13.8787 - 11.8285) * scale,
      center.dx + (10.0342 - 12.0) * scale,
      center.dy + (12.7455 - 11.8285) * scale,
    );
    speakerPath.cubicTo(
      center.dx + (10.0136 - 12.0) * scale,
      center.dy + (12.4249 - 11.8285) * scale,
      center.dx + (10.0005 - 12.0) * scale,
      center.dy + (12.1102 - 11.8285) * scale,
      center.dx + (10.0005 - 12.0) * scale,
      center.dy + (11.8285 - 11.8285) * scale,
    );
    speakerPath.cubicTo(
      center.dx + (10.0005 - 12.0) * scale,
      center.dy + (11.5467 - 11.8285) * scale,
      center.dx + (10.0136 - 12.0) * scale,
      center.dy + (11.2321 - 11.8285) * scale,
      center.dx + (10.0342 - 12.0) * scale,
      center.dy + (10.9115 - 11.8285) * scale,
    );
    speakerPath.cubicTo(
      center.dx + (10.107 - 12.0) * scale,
      center.dy + (9.77806 - 11.8285) * scale,
      center.dx + (10.1432 - 12.0) * scale,
      center.dy + (9.21056 - 11.8285) * scale,
      center.dx + (10.5439 - 12.0) * scale,
      center.dy + (8.93982 - 11.8285) * scale,
    );
    speakerPath.close();
    ...
    // 🔹 3 Static Ordered Waves (Small -> Medium -> Big) with disappearing fading logic
    const waveCount = 3;
    const double baseRadius = 8.5;
    const double radiusStep = 3.5;

    final double currentProgress = progress ?? 0.0;

    for (int i = 0; i < waveCount; i++) {
      final double staticRadius = baseRadius + (i * radiusStep);
      final double wavePhase = (currentProgress - (i * 0.25)) % 1.0;
      final double opacity = (wavePhase < 0.5)
          ? wavePhase * 2
          : (1.0 - wavePhase) * 2;
      ...
      final rect = Rect.fromCircle(center: center, radius: staticRadius);
      ...
      canvas.drawArc(rect, leftStartAngle, sweepAngle, false, wavePaint);
      canvas.drawArc(rect, rightStartAngle, sweepAngle, false, wavePaint);
    }
  }
